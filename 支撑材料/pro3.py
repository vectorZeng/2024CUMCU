...
def total_profit_cal(n, BUY,  components, semi_products, final_product, detect_components,
    ...
    # 计算乘积并存储在 n_semi 中
    updated_defective_semi_rate[0] = 1 + (0.1 - 1) * np.prod(standard_rate[0:2])  # 第1到第3个元素的乘积
    updated_defective_semi_rate[1] = 1 + (0.1 - 1) * np.prod(standard_rate[3:5])  # 第4到第6个元素的乘积
    updated_defective_semi_rate[2] = 1 + (0.1 - 1) * np.prod(standard_rate[6:7])  # 第7到第8个元素的乘积
    ...
    updated_defective_final_rate = 1 + (0.1 - 1) * np.prod(semi_standard_rate[:])
...
# 将结果保存到Excel文件
df.to_excel('问题三决策方案结果.xlsx', index=False)
...
plt.savefig("pics/问题三：不同决策方案的总成本折线图.eps", format='eps')
plt.savefig("pics/问题三：不同决策方案的总成本折线图.png", format='png')
plt.show()
...
plt.grid(True)
plt.legend(fontsize=20)
plt.savefig("pics/问题三：不同决策方案的总利润折线图.eps", format='eps')
plt.savefig("pics/问题三：不同决策方案的总利润折线图.png", format='png')
plt.show()
...
plt.grid(axis='y')
plt.savefig("pics/问题三：总利润最大的10个决策方案直方图.eps", format='eps')
plt.savefig("pics/问题三：总利润最大的10个决策方案直方图.png", format='png')
# 显示图表
plt.show()
top_10_profit.to_excel('问题三：决策方案中利润最高的10个.xlsx', index=False)
...
plt.tick_params(axis='both', labelsize=20)
plt.grid(axis='y')
plt.savefig("pics/问题三：总成本最大的10个决策方案直方图.eps", format='eps')
plt.savefig("pics/问题三：总成本最大的10个决策方案直方图.png", format='png')
# 显示图表
plt.show()
top_10_profit.to_excel('问题三：决策方案中成本最高的10个.xlsx', index=False)
